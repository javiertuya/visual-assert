...
namespace Giis.Visualassert
{
	public class TestDiffLocation
	{
		...
		{
			VisualAssert va = new VisualAssert();
			NUnit.Framework.Legacy.ClassicAssert.AreEqual("Expected is empty", va.GetDiffLocation(string.Empty, "a b\nc d"));
			NUnit.Framework.Legacy.ClassicAssert.AreEqual("Actual is empty", va.GetDiffLocation("a b\nc d", string.Empty));
			NUnit.Framework.Legacy.ClassicAssert.AreEqual("No diffs", va.GetDiffLocation(string.Empty, string.Empty));
		}
	}
}
